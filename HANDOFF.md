...
**Next:** the **pack-scoped cited playbook** retrieval (the moat plumbing — reuse the RAG engine per pack field) + content-ops to gather/cite the playbooks (biggest effort, but the Pro value); then the UI pieces **mockup-first** (two-door entry, freemium showcase, journey-view real-faces + per-pack stage-name upgrade) and, coordinated with the sibling (owns `home.tsx`), routing a founder into the Tasks tab after creation. Fast-follow still open: editable presets (skip/add/reassign/delete) + per-stage playbook hook.

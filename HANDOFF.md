...
**Next (still mockup-first — ZERO UI code until approved):** the Restaurant pack CARD in the picker (new "Local & Services" category — blueprint data ready, card is client UI) + deeper pack-picker preview (UX-01/02); the fast-follow **editable presets** (skip/add/reassign/delete on a journey task) and a per-stage **playbook hook**. Also flagged for coordination (sibling owns `home.tsx`): routing a founder straight into this Tasks tab after pack creation.

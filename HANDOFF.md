...
**Does the learning loop make agents smarter? Measured: no, not turn-to-turn.** Verified the self-improving loop is genuinely built + wired — the AKL (`server/knowledge/akl/`, canon-promotion with governance + rollback), growth-loop lessons (LEARN-01 in `server/ai/qualityLessons.ts`, default-on, injected into both prompt paths), `skillLearner`, personality evolution, trust scoring. But `scripts/eval-learning-loop.ts` (real path: peer-review lesson → seeded `peer_review_feedback` event → `getQualityLessons` injection, model held constant, 2 samples/arm, 6 roles) shows the growth-loop lessons move quality **+0.08/dim, inside the noise** (per-role Δ stdev 3.42 vs mean 0.50) and hurt precision roles. And the **AKL — the strong mechanism — is DORMANT**: `shouldAllowWebCalls()` = `ENABLE_WEB_IN_PROD_MODE` (default false, unset, no search provider) → `runRoleScopedResearch` returns blocked → `akl/runner.ts:98` early-returns `promoted:false`, so it detects + logs gaps but never learns (likely a deliberate cost/prompt-injection safety gate). This confirms the audit's "remembers but doesn't get smarter" with numbers. Corrected an old worry from `project_intelligence_layer_audit`: **RAG is confirmed LIVE** (`RAG_ENABLED` default on; cross-role matrix verified pulling Marketing + UX + Finance for a pricing question). Committed `bb90407`.

**Outcome-based growing-playbook PLAN written, NOT built** (`.planning/growth-loop-outcome-based-PLAN.md`, commits `6091148` + `608b6f7`). Learns from Hatchin's OWN successful work, so — unlike the dormant AKL — it needs **no web**. Three gates: real outcome signal / repetition / gatekeeper+rollback (the last already exists as AKL governance/promotion). Grounded in behavioral outcome signals already in code (deliverable kept-vs-reverted + editsCount + resolved-annotations; task done-vs-reopened; Keep/Refine card; thumbs), reuses the RAG matrix for storage/retrieval. Space is a non-issue (tiny lessons + dedupe + retrieve-only-the-relevant-few). **Sequenced: Phase 1 = capture the outcome signal as a read-only SCOREBOARD** (does the product help users? — the "zero live quality metrics" gap, cheap + pre-launch-safe); **Phase 2 = the learning layer, DEFERRED** until the scoreboard shows patterns AND there is user volume. Honest ceiling recorded: a frozen model means experience gives better notes, not a smarter brain.

**Live multi-pass proof** (2026-08-26). Ran the role-selective multi-pass (`883c7fd`) live on an isolated `:5099` server (real DeepSeek writer + free Groq critic), through the REAL `POST /api/hatch/chat` endpoint, before/after blind-judged: best-case (3 short chat questions on creative roles) overall **3.61 → 4.67 (+1.06)**, every dimension up; the realistic **34-role average stays +0.17**. Coverage confirmed: the feature applies to **all non-precision roles automatically** via `creativityMattersFor(role)` (reads `getRoleIntelligence(role).category`; PRECISION_CATEGORIES finance/legal/data/quality/engineering/operations skip), and **auto-extends to any NEW agent by its category with zero code** (caveat: a brand-new category defaults to *getting* it — add it to the precision set if it should skip). Still gated OFF (`MULTIPASS_ENABLED`), non-streaming path only, nothing merged. `scripts/demo-multipass-live.ts`; artifact `claude.ai/code/artifact/cb991b0b-8f30-40e9-ab2a-df5fe77575f8`.

**Full backup** (2026-08-25). Tag `backup/full-snapshot-2026-08-25` at HEAD + pushed branch + tag to the dedicated `backup` remote (`hatchin-private-backup`); local `~/hatchin-backups/2026-08-25/` holds a verified 85M all-refs bundle and a 317M full-working-tree tar (includes the uncommitted sibling work + `.env`, which git never backs up). Restore: `git checkout backup/full-snapshot-2026-08-25`, `git clone <bundle>`, or `tar xzf <tar>`.

**Note:** this branch also carries concurrent SIBLING commits this window (external-repo borrow-catalog backlog docs, entry-bundle perf halving, sign-in/404/hero-v5 landing redesign, DESIGN.md brand-spec injection, no-dash hardening, a bull/bear debate engine) — a parallel workstream, not detailed here.
...
**Journey view (BIAB-1 UX slice) shipped + approved 2026-08-10 — the visible payoff of increment 1.** Mockup-first gate followed properly: published a clickable prototype (`artifact/555930c6`, Today-vs-Journey toggle, real SaaS data), user approved + locked three build decisions (core + live working-state; reuse the existing Artifact panel; soft guidance, never gates). Then built `client/src/components/sidebar/JourneyView.tsx` (NEW) + wired it into `TasksTab.tsx` (renders the staged Journey when a project has `metadata.stage` tasks, else the legacy board). Staged path (Get set up → Build → Launch → Grow) with per-stage progress + plain-word pills (You're here / Done / Up next), a "Your first 3 moves" starter with the Pro/autonomy-gated "Let the team start" CTA (reuses `delegate_todos`), role-colored teammate avatars incl. the new Juhi/Ira, a live "Working on it now" state (`useAgentWorkingState` + task `in_progress`), and each producing task's doc chip that opens the REAL Artifact panel (reuses the existing `open_deliverable` event → home.tsx). **Zero sibling-owned files touched** — every side-effect uses an existing window event, so no `home.tsx`/`CenterPanel.tsx` edits. Verified live on my own :5021 memory server via an independent headless Playwright run (own browser, not the locked MCP one; sibling :5001 + my prior :5019 untouched, :5021 killed after): Journey renders faithfully, doc chip opens "SaaS Business Plan" with the seeded scaffold, screenshots captured. tsc clean.

**Front-door strategy converged (2026-08-10, via ~6 iterated mockups; user pushed hard on "is this the best?"):** the new-project entry becomes **two separate doors** — "Start with your idea" (FREE: Maya assembles a team + plan for ANY idea) and "Start from a pack" (packs shown prominently as a **Pro** feature). Packs are **freemium**: some free + usable, deep/premium ones Pro (preview free, upgrade to launch). The moat (user's sharpest point: "what stops them just adding the team?") is NOT the roster, it is the **cited field playbook + documents-built-to-spec + verified orchestration** the Pro packs carry — the preview teases that depth and gates the mastery. Intent-first must never dead-end, so an idea with no matching pack gets a **generatively assembled** team. Decisions recorded: **packs = freemium (Pro is the deep playbook layer)** (updates the earlier "free for everyone" lean). Mockups (clickable, real avatars inlined): journey `555930c6`, picker `6634d3a5`, intent entry `41c3aca3`, two-door `7f93ebb0`, Pro-depth `f1d8578d`.

**Server foundation shipped (2026-08-10, server-only, no UI gate, my files only):** `shared/packBlueprints.ts` gains a `tier: 'free'|'pro'` field (SaaS free taste, Restaurant Pro — a one-field product decision) + `packCatalog()`/`summarizeBlueprint()`/`getPackTier()`. New `server/starterPacks/packRouter.ts` = the "server brain": `classifyIdea` (cheap keyword pass with an explicit `// LLM-SEAM` for the drop-in LLM upgrade), `routeIdea` → `{mode:'pack'|'assemble'|'clarify'}`, and `assembleBlueprint(idea)` producing a real seedable blueprint from the universal team + 4-stage lifecycle + known deliverable types (the generative fallback). `storage.initializeAssembledProject(projectId, idea)` (both impls) seeds an assembled idea through the SAME `seedPackBlueprint`. New `server/routes/packs.ts`: `GET /api/packs/catalog` + `POST /api/packs/route`, registered in routes.ts. Verified: `scripts/test-pack-router.ts` 27/27 (routing, assemble shape, tiers, and an assembled weird idea seeding a full guided project: 6 agents / 10 staged tasks / 5 linked docs) + live HTTP on own :5022 (catalog tiers, coffee-shop→restaurant pack, wedding-photography→assemble, not-sure→clarify, unauth→401), then killed; tsc clean.

**Moat plumbing shipped (2026-08-10, server-only, no UI gate, my files):** a pack-seeded project's agents now get the pack's FIELD PLAYBOOK injected at chat time (the defensible Pro depth). `shared/packBlueprints.ts` gains a `playbook: { field, primer, frameworks[] }` on both flagships (real framework NAMES only — no baked-in stats; specifics grounded in RAG at answer time) + summary carries `frameworkCount`/`field`. `projects.executionRules.packId` (type-only) stamps which pack a project came from; the seeder writes it (curated packs + 'assembled'). New `server/starterPacks/packPlaybook.ts`: `getProjectPackId`, `renderPackPlaybookBlock` (curated field directive + cite-or-admit, '' for assembled/unknown/non-pack), `boostQueryForPack` (biases RAG retrieval toward the field). Wired into BOTH `openaiService` paths (streaming + non-streaming) next to the role-knowledge matrix, gated on the project being pack-seeded. Verified: `scripts/test-pack-playbook.ts` 20/20 (block content, gating, stamp, boost, summary) + a **deterministic runtime capture** (SaaS project → 964-char FIELD PLAYBOOK injected; plain project → 0, correctly gated) + a **live real-LLM vibe-check via Gemini** (`scripts/test-pack-playbook-live.ts`): same PM + same "how do I prioritize features?", the **SaaS-pack agent reached for RICE (Reach/Impact/Confidence/Effort)** while the **plain agent gave generic MoSCoW/value-effort** advice — the moat, proven. (Groq was rate-limited to mock, 429 free-tier daily cap — the fail-safe chain worked; used Gemini for the real reply.) tsc clean.

**Next:** **content-ops** to gather + cite the flagship field playbooks (deepen the Pro value beyond framework names — the biggest effort); then the UI pieces **mockup-first** (two-door entry, freemium showcase, journey-view real-faces + per-pack stage-name upgrade) and, coordinated with the sibling (owns `home.tsx`), routing a founder into the Tasks tab after creation. Fast-follow still open: editable presets (skip/add/reassign/delete) + per-stage playbook hook.
...
**What landed (all on branch, nothing merged, additive + fail-safe):**
- **Deep corpus for all 31 roles.** An external Claude Code run (driven by `server/knowledge/rag/seed/GATHERING-HANDOFF.md`) gathered 390 authoritative sources into 13 new `batch-*-deep.json` seed files (real fetched URLs, faithful extracts, anti-patterns), then embedded them. I cross-verified the whole thing against live code and DB before trusting it: corpus present, retrieval faithful, correct sources rank first.
- **Production embedder decision.** The external run embedded with local Ollama (`nomic-embed-text`, 768d), which is free but LOCAL ONLY: a deployed server cannot reach `127.0.0.1:11434`, so RAG would silently go empty in production. The user funded and pasted an OpenAI key; I re-embedded ALL 31 roles under OpenAI `text-embedding-3-small` @768d into one consistent vector space in Supabase `role_knowledge`. Cost: a few cents one-time (about ₹1 to ₹2), pennies per month after.
- **Final corpus: 3,875 chunks across 31 roles.** Coverage 31/31, `test-rag-coverage` 36/36, matrix `test-rag-crossrole` 5/5.
- **Juhi = Finance Analyst is the 31st registered role** (`shared/roleRegistry.ts` + `shared/roleIntelligence.ts`, category "finance", full FP&A depth), 103 chunks, reachable via `ragRoleKey("Finance Analyst") = "finance-analyst"`.
- **Fixed a stale coverage test:** `scripts/test-rag-coverage.ts` hardcoded 30 roles and kept flagging Juhi as a stray key; it now derives the role list from `ROLE_DEFINITIONS` so it can never drift again (36/36).
- **Recalibrated the relevance floor for OpenAI.** OpenAI cosine scores run lower than Ollama's (relevant ~0.45 to 0.63, noise ~0), so the old `RAG_MIN_SCORE=0.55` was silently starving the matrix (an onboarding query collapsed from 6 sources to 1). Set `RAG_MIN_SCORE=0.40`; full cross-role diversity restored with no noise.
- **UX course restored + extended.** The re-embed wiped the local UX course (it lives outside the seed files); re-ran `scripts/ingest-ux-course.ts` under OpenAI, which ingested all 35 topic files (2,382 chunks; local Ollama had managed only 2 before an HTTP 500). UX Designer now holds 2,447 chunks.
- **Pluggable embedder** (`server/knowledge/rag/embeddings.ts`): `RAG_EMBED_PROVIDER` = openai | ollama | gemini, all 768d, one-vector-space rule enforced.

**`.env` (gitignored, not committed):** `RAG_EMBED_PROVIDER=openai`, `RAG_MIN_SCORE=0.40`, `OPENAI_API_KEY` added. **User to rotate the key** (it came through chat).

**Live proof (verify-in-runtime):** started my own isolated server on `:5017` (memory mode, autonomy off, OpenAI RAG; the sibling's `:5001` untouched, no process kills), dev-login, created a project, asked one SaaS onboarding question. Alex (Product Manager) answered using RICE (Product Manager corpus) plus the "aha moment"/activation and Casey Winters' Key Action Framework (Growth Marketer corpus): cross-role knowledge fused in a real conversation. Screenshot in scratchpad `rag-live-proof.png`. The `:5017` server was left running for the user.

**Known gap (not built, UI-approval-gated):** `client/src/components/chat/SourceChips.tsx` builds citations by scanning the reply text for a URL; the model named the frameworks but did not echo a URL, so no chip rendered. The knowledge is used, just not surfaced as a clickable source. Recommended fix: render chips from the RETRIEVED sources (we know exactly what was injected), not from URLs parsed out of the reply. Client change, so it needs the UI-change approval gate first.

**Not done yet:** wire RAG into production + merge (nothing merged; now OpenAI so it is production-capable); the citation-chip improvement; STATE.md / ROADMAP.md / REQUIREMENTS.md / COMPLETE-GUIDE entries (RAG is a real feature so it warrants them, deferred because STATE/ROADMAP/package.json carry a sibling session's uncommitted work per `feedback_parallel_work_safety`, do not stage).

**Files (mine, on branch):** `server/knowledge/rag/{embeddings,store,retriever,ingest}.ts`, `scripts/{ingest-all-role-knowledge,ingest-ux-course,test-rag-coverage,test-rag-crossrole,eval-role-competence,...}.ts`, `server/knowledge/rag/seed/*.json` (13 new deep batches), `shared/roleRegistry.ts` + `shared/roleIntelligence.ts` (Juhi), `client/src/components/chat/SourceChips.tsx` (earlier). `.env` local only.

**Continuation (same session): three new roles added after skipping the C-suite.** The user asked whether we were missing CEO/CTO-type roles. Decided (CTO recommendation, user agreed) to SKIP the C-suite: the user is the CEO, and CEO/COO/CMO/CPO/CFO mostly duplicate existing roles (Maya, Operations, Marketing and Growth, Product, Juhi) and dilute routing, while the real gaps are whole functions with zero coverage. Added those gaps as full roles: **Legal Counsel (Ira), Sales Lead (Dana), Customer Success Manager (Tess)**, each a full `roleRegistry` + `roleIntelligence` entry with a distinct voice and reasoning profile (no `client/src` edit, so no UI gate). Gates green: voice 8/8, pushback 50/50, reasoning 272/272, tsc clean (34 roles now); added `legal`, `sales`, `customer-success` to the reasoning-test category set. Three parallel research subagents gathered 55 faithful real-URL sources (Legal 18: GDPR, CCPA, YC SAFE, USPTO, copyright.gov, IRS, FTC eCFR via Cornell LII, Common Paper, Holloway; Sales 18: MEDDIC, Gong data, Challenger, JOLT, Winning by Design; CS 19: Gainsight, Lincoln Murphy, Kellblog, Bessemer, SaaStr, First Round), validated (valid JSON, correct role, all http URLs, zero em/en dashes). Embedded under OpenAI via `scripts/ingest-role-knowledge.ts` per file (replaceRoles touches only that role, so the 2,447-chunk UX course was preserved). New chunks: Legal 90, Sales 60, CS 82. Total 4,107 chunks, coverage 34/34, matrix probe 3/3 (a churn question fuses Customer Success + Finance; a legal question pairs Legal + UX privacy ethics; a sales question pairs Sales + Marketing). New seed files: `batch-legal-deep.json`, `batch-sales-deep.json`, `batch-cs-deep.json`. Also decided (user product call): do NOT make citations always visible, it would crowd replies and fight the colleague tone; knowledge shows as natural framework-naming, revisit a subtle on-demand Sources link only if real users ask.

---

## 2026-08-01 — Pre-Launch Hardening pass (Tier-0 launch blockers) (server workstream)

User pasted a **Master Pre-Launch Audit** (88 findings across security S1-S6, engineering A-G, infra, frontend, compliance, a11y, billing, bug-hunt, API exposure, account/content safety) with a tiered roadmap (Tier 0 launch blockers, Tier 1 fast-follow, Tier 2 maturity). Progression: "cross verify first" → "fix it" → "but it flagged so many things" → "yes fix everything" → **"always test your fixes first, then if it's good then proceed"** (now a durable rule: per-fix test-gating, never batch-then-test; spin up my OWN server on a spare port and shut it down after, never touch a sibling's process). ~88 findings dedupe to far fewer distinct fixes; only ~8 truly block launch. I cross-verified every Tier-0 blocker against live code/DB before fixing, then closed **every code-doable one**, each self-reviewed on the user's behalf ("go ahead, I can't review it, test it and review it on my behalf") and tested before commit. Server-side only, path-disjoint from the parallel v2.1-UX/Slack work.

**Shipped (7 fix commits + 3 docs):**
- `cc0fb6f` **cost brakes + IDOR + account-takeover + hot-query cap.** New `server/billing/costGuard.ts` (`checkCostGuard`): per-user rate limit + per-user daily $ cap + global daily $ kill, **independent of `FEATURE_BILLING_GATES`** (which defaults off), fail-open on a DB read error, in-memory rate limiter, wired at the WS `send_message_streaming` handler (before the message-safety cap). IDOR on `POST /api/personality/feedback` (added ownership check via getProject + Zod validation). Account-takeover (ACCT-1): `upsertOAuthUser` now does a strict `getUserByProviderSub` lookup, removed the email fallback, throws explicit `OAUTH_EMAIL_CONFLICT` when the email belongs to a different account (mapped to `/login?error=email_in_use` in the callback); `email_verified !== true`. Hot-query cap: `getMessagesByConversation` rewritten to SQL ORDER/LIMIT/cursor with `MESSAGES_HARD_MAX` 500 + new `messages_conversation_created_idx` composite index + id tiebreaker.
- `2cf7d32` **DOS-3** — `enforceCostGuard()` helper applied to every LLM-spending HTTP endpoint (deliverable generate/iterate/reader-test/packages). Live: 2nd generate at cap=1 returned 429.
- `25b41e4` **health check + real DB ping.** `server/routes/health.ts` + `storage.ping()` (SELECT 1); returns **503 only on infra/DB down, not on LLM-provider down**. `fly.toml` check points at **`/health`, NOT `/api/health`** which 401s behind `app.use('/api', requireAuth)`, this catch prevented a deploy that would have 401-killed every instance.
- `319cf8f` **spend alarm** — fires once/day in the warn band before the global kill, via `maybeFireSpendAlarm` + `OPS_ALERT_WEBHOOK_URL`.
- `32a17fb` **structured error tracker.** New `server/observability/errorTracker.ts` `captureException(err, context)`: JSON log + Sentry auto-forward via a computed specifier `'@sentry/'+'node'` so it typechecks WITHOUT the package installed. Wired into the Express error handler (+ generic 500 body in prod, SECRET-2, no stack leak) + uncaughtException + unhandledRejection.
- `22c7db1` **BUG-3 interim** — `recordEstimatedUsage()` in `usageTracker.ts` records a conservative per-generation cost estimate fire-and-forget after `handleMultiAgentResponse`, so multi-agent team turns (which never set `llmMetadata`) become visible to both $ caps; integer-cent rounded so free Groq + sub-cent flash turns self-skip. **Correction to the spec:** the safety-intervention path was deliberately NOT wired, `buildClarificationIntervention` is a template with no LLM call, so an estimate there would be phantom cost. The exact measured-token version rides the ARCH-1 orchestrator extraction.
- `f427078` `.audit-agent-intelligence/TIER0-REMAINING.md`, `2662002` `.audit-agent-intelligence/MIGRATION-BASELINE-RUNBOOK.md`, `5aa99ce` `V2.2-FIXES-AUDIT.md`.

**Config knobs (safe defaults):** `COST_GUARD_MSGS_PER_MIN` 40, `COST_GUARD_USER_DAILY_CENTS` 500, `COST_GUARD_GLOBAL_DAILY_CENTS` 2000, `COST_GUARD_GLOBAL_WARN_CENTS` 1400, `MESSAGES_HARD_MAX` 500, `COST_EST_PROMPT_TOKENS_PER_GEN` 3500, `SENTRY_DSN` (optional), `OPS_ALERT_WEBHOOK_URL`.

**Verify-first caught 3 real problems before they shipped:** the `/health` 401 deploy-killer; the `users.email` UNIQUE conflict path (a MemStorage test had masked it); the phantom-cost safety estimate. Tests (local npm is broken, run via `./node_modules/.bin/tsx`): `test-cost-guard` 14/14, `test-acct-takeover` 6/6, `test-messages-query` 8/8 live, `test-error-tracker` 9/9, `test-estimated-usage` 11/11, `verify-estimated-usage-live` 2/2 live.

**THREE Tier-0 items remain, none code-doable from here** (saved to memory `project_prelaunch_hardening_remaining` so I remind the user; full status in `TIER0-REMAINING.md`):
1. **0.5 Backups: enable Supabase PITR** (~2-min dashboard toggle, needs Supabase Pro ~$25/mo ~₹2,150, then one restore drill). **DO THIS FIRST**, biggest risk reduction, least effort, the last data-loss gap.
2. **0.4 Migration-safety baseline** — live schema was built with `drizzle-kit push` and `migrations/` covers only 7 of 21 tables, so a rename/removal in `schema.ts` can silently `DROP COLUMN`. Full staging-first, backup-gated, copy-paste runbook at `MIGRATION-BASELINE-RUNBOOK.md`. Needs a staging DB + clean npm/package.json.
3. **0.6 Sentry install** — `npm i @sentry/node` + set `SENTRY_DSN` in Fly secrets; the error tracker auto-forwards the moment package+DSN exist, no code change. Needs working npm.

**Net:** money and data are protected at runtime (cost brakes + spend alarm + health check + error capture); the remaining depth (PITR + migration safety) needs the user's Supabase action + a staging rehearsal, and Sentry needs a working npm. **Constellation:** CLAUDE.md footer + this HANDOFF updated per the user's explicit ask; STATE.md + ROADMAP.md + package.json intentionally NOT staged (sibling's uncommitted v2.1-UX + Slack work). Nothing merged.

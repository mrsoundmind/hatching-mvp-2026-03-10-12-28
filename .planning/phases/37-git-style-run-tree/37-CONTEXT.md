...
| `eventLogger.recordEvent` | `server/autonomy/events/eventLogger.ts:75` | Augment OR sibling — when an autonomy event fires, also write a step row if it's a task / handoff event type |
...
| `handoffOrchestrator.routeHandoff` | `server/autonomy/handoff/handoffOrchestrator.ts` | Step-start with parent_step_id = caller's current step at handoff begin |
...
- **D-06:** `taskExecutionPipeline.executeTask` is instrumented at 3 points: (a) on function entry, lookup-or-create the run (by `traceId` from the calling context), then `startStep` with stepType='task', returning a `stepId` held in local var; (b) on successful return after deliverable persistence, `completeStep(stepId, { deliverableId, deliverableVersionId, scoreDelta })` where `scoreDelta = currentVersion.rubricScore.total − priorVersion.rubricScore.total` (null if no prior); (c) on any throw, `failStep(stepId, error)` before rethrow.
- **D-07:** `handoffOrchestrator.routeHandoff` instrumented: when a handoff is decided, write a step row with `stepType='handoff'` and `parentStepId = sourceStep.id`. The downstream agent's `executeTask` then writes a `'task'` step with `parentStepId = handoffStep.id` — so the tree shows source-task → handoff-link → next-task as 3 generations.
...
- **D-17:** Backfill is a one-shot script (`scripts/backfill-run-tree.ts`), idempotent. Logic: for each project, find all distinct `trace_id` in `autonomy_events` that have NO corresponding `autonomy_runs` row. For each, create a run row, then walk events in timestamp order writing step rows. Inferring `parent_step_id`: for handoff events with `payload.previousAgent` set, the parent is the most-recent step with that agent's `agentId`. For task events without explicit parent, parent is the run's root step.
- **D-18:** Backfill quality is best-effort, not perfect — some historical events lack the structured payload Phase 37 expects. Run rows from backfill get `metadata.backfilledFrom = 'autonomy_events'` so we can identify them in QA. The UI treats them identically. Worst case for a lossy backfill: tree shows a flat list of steps under the run root instead of a real hierarchy. Acceptable per "make autonomous Hatch work browsable" goal — flat is still better than nothing.

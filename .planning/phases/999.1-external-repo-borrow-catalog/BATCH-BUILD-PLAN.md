...
> Done on-branch (not merged). New `server/ai/brandSpec.ts` (single canonical spec: voice + writing conventions + visual tokens for the future export wave), injected into BOTH deliverable prompt paths (`buildGenerationPrompt` + `iterateDeliverable`). Gated by `BRAND_SPEC_ENABLED` (default on). Verified in the REAL generation path via the capture provider: `scripts/test-brand-spec.ts` 25/25 (block content + on/off gate + injected-when-on + absent-when-off + mechanical dash guarantee), tsc clean.
> **Hardening (2026-08-26):** the no-em/en-dash rule is now a GUARANTEE for documents, not just a request. `enforceBrandStyle()` in brandSpec.ts delegates to the canonical `stripDashes` (now exported from responsePostProcessing.ts, the same stripper chat uses) and runs over generated + iterated content before persist, gated by the same flag. Live proof: a blog post that had 5 dashes (spec off) to 1 (prompt only) is now 0 (prompt + strip).

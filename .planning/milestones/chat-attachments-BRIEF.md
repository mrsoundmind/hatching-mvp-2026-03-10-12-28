...
### Phase 3 (Feature A: docs in chat) . M-L . SERVER DONE (2026-08-06); UI is the remaining piece
Composer paperclip (reuse `DocumentUploadZone` logic) or drag-drop onto the composer. **UX MODEL LOCKED 2026-08-06 (user decision): ONE TAP, no popup.** Attaching scopes the file to the conversation (persists, reusable by any agent in THAT chat on any later question). A small **optional "Add to project brain"** link on the file chip promotes it to project-wide (every agent, every chat) for the rare case someone wants that. The forced two-choice menu was rejected as friction/confusion on the core action; the Brain tab already owns deliberate project-wide knowledge. New `POST/GET/DELETE /api/conversations/:id/attachments` (hardened multer, cost-guarded) — SHIPPED. Still to build UI-side: paperclip + file chip (filename, type badge, size, Uploading→Ready state, optional Add-to-brain link, × to remove) in the composer AND on the sent message bubble; a "promote to brain" server action (UPDATE scope='brain', conversation_id=NULL on the doc + its chunks). Optionally a nullable `messages.attachments` JSONB for bubble display (or fetch via the GET endpoint to avoid touching the Drizzle `messages` table). Multi-agent: retrieved chunks are shared context for the turn. UI-change protocol applies (composer + bubble are `client/src`); prototype approved (one-tap model), published at the artifact URL in the 2026-08-06 session.

**Retention (LOCKED, and answered for the user 2026-08-06):** attachments are NEVER auto-deleted. Both scopes persist in Postgres (text + embeddings) and stay searchable by agents on later questions, until the user removes them with the × (DELETE endpoint → cascade-wipes the chunks too; this is also the privacy / "forget this" / compliance-erasure path). No TTL, no cron. "Ephemeral" is a misnomer inherited from the code: it means conversation-scoped, NOT time-limited. Do not ship UI copy implying a file is temporary or "not saved" — it is saved, just kept to one chat.
...
- **Attach is one tap, scoped to the conversation by default** (LOCKED 2026-08-06, superseding the earlier "explicit two-choice" design). Project-wide is an optional secondary action ("Add to project brain" on the chip), never a forced modal. Rationale: forcing a scope choice on every upload is friction on the core action and "project brain" is our jargon; the Brain tab already owns deliberate project-wide knowledge. Files persist either way (see Retention above); the choice is about WHERE the file is found later, not whether it is kept.

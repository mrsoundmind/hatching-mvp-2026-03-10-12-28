...
**Plans**: 5 plans
  ...
  - [ ] 38-02-PLAN.md — **CRITICAL — blocks any autonomous ship.** Safety scorer destructive-intent detection: add destructive-verb pattern set (`delete`, `wipe`, `reset`, `remove all`, `start over`, `nuke`, `erase`, `destroy`) to `server/ai/safety.ts` with weighted risk contribution; refactor `evaluateSafetyScore` to return `executionRisk ≥ 0.70` on match; add regression test asserting live "delete all my data" fires the approval card at level-4; verify D-11..D-13 grep=7 invariant preserved. Closes ALWY-04.
  - [ ] 38-03-PLAN.md — Maya voice snap at level-4: extend `AUTONOMOUS_DIRECTIVE_BLOCK` in promptTemplate.ts with a Maya-specific clause overriding her "I keep coming back to..." opener; add prompt-snapshot test case for Maya autonomous voice ("Here's what I'd do: X. Because Y. Flag if wrong."); verify via live vibe-check. Closes ALWY-05.

...
- 📋 **Outcome-Based Growing Playbook ("agents get smarter from experience")** — PLAN written, NOT started (2026-08-24). Full design: [growth-loop-outcome-based-PLAN.md](growth-loop-outcome-based-PLAN.md). Distills lessons from Hatchin's OWN successful work (needs NO web, unlike the dormant AKL), gated by 3 filters (real outcome signal / repetition / gatekeeper+rollback — the last already exists as AKL governance), grounded in behavioral outcome signals already in code. **Sequenced: Phase 1 = capture the outcome signal as a read-only scoreboard (does the product help users? — the "zero live quality metrics" gap, cheap + pre-launch-safe); Phase 2 = the learning layer, deferred until the scoreboard shows patterns + there's user volume.** Context: the 2026-08 investigation measured that the *existing* loop remembers but does not get smarter turn-to-turn (growth-loop lessons ~0; AKL dormant because web is gated off), and that the role-selective multi-pass (`883c7fd`, gated OFF) lifts short chat answers (+0.17 avg / +1.06 best-case, proven live). See CLAUDE.md footer + HANDOFF.md 2026-08-24 to 08-26.
- 🔨 **In-chat document editing** — SHIPPED on-branch (2026-08-28, nothing merged). Attach a file in chat, tell an agent to edit it, get it back as a download in the same format (all six: Word/PDF/Markdown/text/Excel/CSV). Server engine `server/documents/*` + edit/download endpoints + a download card in the agent reply; the only new UI is that card. Proven live end-to-end. Commits `5ebc25d`/`071e972`/`3c4114f`. See CLAUDE.md footer + HANDOFF (2026-08-26 to 08-28).

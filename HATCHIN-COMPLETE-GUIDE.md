...
> **Last updated**: 2026-09-01 | **Branch**: `feat/v2.2-intelligence-fixes` | **Version**: v2.0 shipped, v2.1 Phase 38 ✅ CLOSED. 2026-07-17 audit remediation complete on branch. Next canonical phase: 39.
...
> **Intelligence & learning — honest status (2026-08, on-branch, nothing merged).** (1) **Knowledge** is real and LIVE: a per-role RAG matrix (7,852 chunks across 34 roles, `RAG_ENABLED` default on) that retrieves the best knowledge across ALL roles for any question — verified pulling Marketing + UX + Finance for a pricing question. (2) **Answer quality** has a *role-selective multi-pass* (`server/ai/multiPass.ts`, `883c7fd`, gated OFF via `MULTIPASS_ENABLED`): after the DeepSeek draft, a free Groq critic scores it against our owned-method rubric and the agent revises, with a rigor gate that can only keep or raise honesty. It lifts short chat answers (+0.17 avg across 34 roles, +1.06 best-case, proven live on `/api/hatch/chat`) and applies to **all non-precision roles + any new agent automatically by category** (precision roles finance/legal/data/QA/engineering/operations skip it, since the extra pass hurts them). (3) **"Getting smarter from experience" is honestly NOT working yet**: the loop (AKL canon-promotion, growth-loop lessons, skillLearner, trait evolution, trust) is built + wired, but measured — growth-loop lessons move quality ~0 (in the noise), and the strong AKL mechanism is dormant because its web research is gated off. The real fix is scoped but not built: `.planning/growth-loop-outcome-based-PLAN.md` (measure-first scoreboard, then a learning layer that distills lessons from Hatchin's own successful work). So the accurate claim is "remembers your project + working style, grounded in a curated cross-role knowledge base, and double-checks itself before it answers" — NOT "gets smarter over time" (yet).

> **Document editing in chat (2026-08, on-branch, nothing merged).** You can attach a document to a chat, tell an agent to edit it ("add a Risks section", "rewrite the intro", "go through and add relevant info"), and the agent replies in chat with the edited file as a **download in the same format you sent** — Word (.docx), PDF, Markdown, or text. It all happens in the chat (no separate screen); the only new UI is a download card in the reply. Under the hood (`server/documents/`): the original file is kept (`conversation_documents.raw_bytes`), extracted to Markdown, edited by DeepSeek (conservative — preserve + add relevantly, never invent facts), then written back to the same format (`docx` package for Word, playwright HTML→PDF for a clean regenerated PDF). Edited outputs are stored (`conversation_edited_documents`) and streamed by a download endpoint. Excel (.xlsx) and CSV are supported too (via the exceljs library; each sheet becomes an editable markdown table, tabular data preserved, formulas/charts not). All six formats — Word, PDF, Markdown, text, Excel, CSV — edit in chat. Proven live end-to-end. This is distinct from the existing "attach a file to ask questions about it" (RAG) — a conservative keyword heuristic routes an edit-style message to the editor, everything else stays normal Q&A.
...
| Work output section | Tasks (below the task list) | [components/sidebar/WorkOutputSection.tsx](client/src/components/sidebar/WorkOutputSection.tsx) — completed task outputs, one-open-at-a-time accordion. Shows the agent who actually executed the task (`metadata.completedByAgentName`, written by `markTaskCompleted`) with their real avatar, and the produced output rather than the task description. Rows with no recoverable agent show a completed-work checkmark instead of a placeholder name |
